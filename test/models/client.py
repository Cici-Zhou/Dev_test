...
"""
添加用于接口测试的client,对于HTTP接口添加HTTPClient,发送http请求。还可以封装TCPClient,用来进行tcp链接，测试socket等等
"""
import requests
...
class HTTPClient(object):
    ...
    def __init__(self,url,method='GET',headers=None, cookies=None):
        """headers:字典。例：headers={'Content-Type':'text/html'},cookies也是字典。"""
        self.url = url
        self.session = requests.session()
        self.method = method.upper()
        if self.method not in METHODS:
            raise UnSupportMethodException('不支持的method:{},请检查传入参数！'.format(self.method))
